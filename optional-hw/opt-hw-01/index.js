/*
WEB PROGRAMMING's OPTIONAL ASSIGNMENT 01
...
//question 01
//List all the planets SG-1 has yet to be visited.
let visited_planets = ( planets.filter(pl => pl["gate"] !== 'Not visited yet') ).map(pl => pl["name"])
console.log("Visited Planets: " + visited_planets.join(" , "))

//question 02
//Count the number of planets that SG-1 needs to visit
let unvisited_planets = planets.filter(pl => pl["info"] !== 'Hostile' && pl["gate"] !== 'Destroyed' && pl["gate"] !== 'Reachable' )
console.log("SG-1 needs to visit " + unvisited_planets.length + " planets.")

//question 03
//List all the allies in case of a Goa'uld attack (All friendly planets with existing Stargate)
let allies_planets = ( planets.filter(pl => pl["info"] === 'Friendly' && pl["gate"] !== 'Destroyed') ).map(pl => pl["name"])
console.log("Allies of Earth: " + allies_planets.join(" , "))
